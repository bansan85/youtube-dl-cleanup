...
{
  return std::find(extension_movies.begin(), extension_movies.end(), ext) !=
         extension_movies.end();
}
...
static bool isFxxx(std::wstring_view ext)
{
  return ext.starts_with(L"f") && ext.length() == 4 &&
         std::all_of(ext.begin() + 1, ext.end(),
                     [](wchar_t c) { return std::isdigit(c); });
}

static bool isPartFrag(std::wstring_view ext)
{
  return ext.starts_with(L"part-Frag") &&
         std::all_of(ext.begin() + 9, ext.end(),
                     [](wchar_t c) { return std::isdigit(c); });
}
...
static bool isMoviesPartialDownloaded(std::wstring_view filename)
{
  ...
  for (auto txt : ints) v.push_back(txt);
  /*
...
*/
  ...
  // .fxxx.webm.part-Fragyyy
  if (v.size() >= 4)
    if (v.size() >= 4 && isFxxx(v.end()[-3]) && isMovie(v.end()[-2]) &&
        (v.back() == L"part" || isPartFrag(v.back())))
    {
      return true;
    }

  // .fxxx.webm.part-Fragyyy.part
  if (v.size() >= 5 && isFxxx(v.end()[-4]) && isMovie(v.end()[-3]) &&
      isPartFrag(v.end()[-2]) && v.back() == L"part")
  {
    return true;
  }
...
int main(int argc, char* argv[])
{
  ...
  {
    std::cout
        << "Usage: youtubedl-cleanup.exe [--dry-run] [-r] path [more_path]\n";
    return 1;
  }
  ...
  while (!folders.empty())
  {
        ...
        else
        {
          ...
          if (isMoviesPartialDownloaded(entry.path().wstring()))
          {
            RemoveFile(entry.path().wstring(), dry_run);
                   ...
                   isMoviesEntiredDownloaded(std::wstring_view{ext}.substr(1)))
          {
            auto root = (entry.path().parent_path() /
    ...
    for (const auto& [file, ext] : file_ext)
    {
      ...
      for (const auto& exti : ext)
      {
        ...
        hFile = CreateFileW(filei.c_str(), GENERIC_READ, FILE_SHARE_READ, NULL,
                            OPEN_EXISTING, FILE_ATTRIBUTE_NORMAL, NULL);

        if (hFile == INVALID_HANDLE_VALUE)
        {
          std::wcerr << "Could not open file \"" << filei << "\", error "
                     << GetLastError() << "\n";
          return 1;
        }
        ...
        times.insert(
            {(static_cast<std::uint64_t>(ftCreate.dwHighDateTime) << 32L) +
